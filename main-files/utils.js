...
export function calculateGrade(down, up, ping) {
    if (!down || !up || !ping)
        return { grade: '-', text: 'Incomplete test' };
    let score = 0;
    ...
    if (ping < 20)
        ...
        score += 10;
    if (score >= 90)
        return { grade: 'A+', text: 'Incredible speed. Perfect for 8K streaming and pro gaming.' };
    if (score >= 80)
        return { grade: 'A', text: 'Excellent. Great for 4K streaming and heavy use.' };
    if (score >= 70)
        return { grade: 'B', text: 'Good. Handles HD video calls and regular use well.' };
    if (score >= 50)
        return { grade: 'C', text: 'Fair. Might struggle with multiple 4K streams.' };
    if (score >= 30)
        return { grade: 'D', text: 'Slow. Basic web browsing and SD video only.' };
